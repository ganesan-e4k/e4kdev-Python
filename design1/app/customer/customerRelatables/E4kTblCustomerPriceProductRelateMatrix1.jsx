...
const E4kTblCustomerPriceProductRelateMatrix1 = ({ showModalMediumCustomerPriceProductMatrix, handleCloseMediumCustomerPriceProductMatrix }) => {
    ...
    const [scrollBottomReached, setScrollBottomReached] = useState(0);
    const [dataSource, setDataSource] = useState([]);
     
    const [Productlist, setProductlist] = useState([]);
    console.log("cghcxhcxjhcxc",Productlist )
    const [Productlist1, setProductlist1] = useState([]);

    console.log("hvjdvdjvdvkjdvdv",Productlist1 )
    const [AllProductlist, setAllProductlist] = useState([]);
    const [newCustomer ,setnewCustomer] =useState(false);
    const dropdownGridCustomerPriceProductRelateRefCustomer = useRef();
    const switchbuttonCustomerPriceProductRelateRef = useRef();
    const [selectProduct,setSelectProduct] = useState('')
    console.log("selected product312321: " + selectProduct)
    const [SelectProductDropDownChange,setSelectProductDropDownChange] = useState('')
    ...
    console.log("gchdvgdvhgjhvbhvb",{
        companyid: Customerselect.CompanyID ||  Customerselect.companyid,
        productid: selectProduct 
    } );

    console.log("bcjdbvjdkvv",standardPriceProductmatrixdataCopy )
...
///////////////// DropDown change
    ...
    //     label: `${product.productid}${product.description}`,
    //     value: product.productid,
    // }));

    // // Initialize dataSource with first 150 items
    // useEffect(() => {
    //     if (items.length > 0) {
    //         const initialItems = items.slice(0, 150);
    //         console.log("Initial slice data", initialItems);
    ...
    //     if (loading || scrollBottomReached * 150 >= items.length) {
    //         return; 
    //     }

    //     setLoading(true); 
    //     console.log('Scrolling done, loading more items...');
    //     const start = 150 * scrollBottomReached;
    //     const end = 150 * (scrollBottomReached + 1);

    //     if (start < items.length) {
    //         setScrollBottomReached(prev => prev + 1);
    ...
    useEffect(() => {
        if (Productlist && Productlist.length > 0) {
            // Only run if Productlist is available and not empty
            const items = Productlist.map(product => ({
                label: `${product.productid}   -   ${product.description}`,
                value: product.productid,
            }));
            ...
            // Initialize dataSource with first 150 items
            // const initialItems = items.slice(0, 275248);
            // console.log("Initial slice data", initialItems);
            setDataSource(items);
        }
     
    }, [Productlist]);  // Runs whenever Productlist changes



    const handleFiltering = (event) => {
        if (!dataSource) return []; // Return an empty array if dataSource is undefined
      
        // Continue filtering logic...
        return dataSource.filter(item => item.label.toLowerCase().includes(event.filter.toLowerCase()));
      };

    // useEffect(() => {
    //     console.log('Updated DataSource:', dataSource);
    // }, [dataSource]);  // Logs when dataSource updates

    // const handleScrollBottomReached = () => {
    //     // Prevent further loading if already loading or if all items are loaded
    //     if (loading || scrollBottomReached * 275248 >= Productlist.length) {
    //         return;
    //     }

    //     setLoading(true);  // Set loading state to prevent duplicate requests
    //     console.log('Scrolling done, loading more items...');

    //     // Calculate the range of items to fetch based on the current scroll state
    //     const start = 275248 * scrollBottomReached;
    //     const end = 275248 * (scrollBottomReached + 1);

    //     // If the start index is within bounds, load more items
    //     if (start < Productlist.length) {
    //         setScrollBottomReached(prev => prev + 1);  // Increment scroll position
    //         const newItems = Productlist.slice(start, end);  // Get the new items
    //         setDataSource(prevData => [...prevData, ...newItems]);  // Append to dataSource
    //     } else {
    //         console.log('No more items to load.');
    //     }

    //     // Simulate loading delay and reset loading state
    //     setTimeout(() => {
    ...
    const handleCreateNewCustomerPriceProduct =() => {
        ...
        console.log('Create new customer price product',AllProductlist) 
       ...
       if (AllProductlist && AllProductlist.length > 0) {
            ...
            const datagrid11 = AllProductlist
                                ...
                                .filter(category => !dataproductbusiness.includes(category.productid));
            console.log('All Product List After filtering: = ', datagrid11)  
            const jsonString = JSON.stringify(datagrid11);
            const Type = typeof(jsonString);
            console.log("cbdhvbndvdvdv",Type )
            console.log('Filtered Products in JSON format:', jsonString);

             setProductlist1(jsonString);                 
    ...
    useEffect(() => {
        console.log('pfefgdfhsdfsd',Productlist)
        if (typeof(Productlist1) === 'string') {
            try {
             
                const parsedProductList = JSON.parse(Productlist1);
                console.log("cghdcdcbdcstringdata", parsedProductList)
                
                setProductlist(parsedProductList);
            } catch (error) {
                console.error('Failed to parse Productlist:', error);
            }
        }
    }, [Productlist1]);
...
const handleProductItemClick = (event) => {
    ...
    if (productid) {
      ...
      console.log('cliking produv=ty ',clickdata )
    ...
    const [isMinimizedCustomerPriceSetup, setisMinimizedCustomerCategory1]= useState(false);
    ...
    const modalDialogclassNameCustomerPriceProduct = isMaximizedCustomerPriceProduct ? 'modal-content modal-fullscreen' : 'modal-content medium-popup';
    const handleMinimizecustomerCategory1page = ()=>{
        setisMinimizedCustomerCategory1(!isMinimizedCustomerPriceSetup);
      };
    ...
    return (
        ...
        <Draggable handle=".e4kmodal-header">
            ...
            {/* <div className={`modal fade ${showModalMediumCustomerPriceProductMatrix ? 'in' : ''}`} style={{ display: showModalMediumCustomerPriceProductMatrix ? 'block' : 'none' }}> */}
            <div className={`modal fade mymodal ${(isMinimizedCustomerPriceSetup === true) ? 'min min-subpopup-footerbar' : 'in'}`} style={{ display: showModalMediumCustomerPriceProductMatrix ? 'block' : 'none' }}>
            <ResizableBox width={resizableWidth} height={resizableHeight} className={modalDialogclassNameCustomerPriceProduct}>
                {/* <div className={modalDialogclassNameCustomerPriceProduct}> */}
                                                    ...
                                                    <div className="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                                                        <div className='popup-top-rightdiv'>
                                                            ...
                                                            <button className="close modalMinimize" onClick={handleMinimizecustomerCategory1page}>
                                                                        <i className={`fa ${isMinimizedCustomerPriceSetup ? 'fa-plus' : 'fa-minus'}`}></i>
                                                                </button>
                                                            <button type="button" className="btn-link" onClick={toggleMaximizeCustomerPriceProduct}>
                                                                {isMaximizedCustomerPriceProduct ? <i className='fa fa-compress'></i> : <i className='fa fa-expand'></i>}
                                                            </button>
                                ...
                                <div className="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                    <div className='form-group master-option'>   
                                      ...
                                      <DropDownList
                                        ...
                                        onChange={handleProductItemClick}
                                        ...
                                        header={headerPrice}
                                        messages={message}
                                        itemHeight={30} // Set the item height for proper rendering
                                        scrolling={true} // Enable scrolling
                                        virtualized={true} // Enable virtualization for performance
