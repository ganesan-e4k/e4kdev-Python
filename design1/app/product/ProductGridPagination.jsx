...
const ProductGridPagination = () => {
  ...
  const CompanyProductGridpage = useSelector((state) => state.selectCompanyid.Companyid);

  useEffect(() => {
    if (status === 'idle') {
      dispatch(fetchProductpage({ after: '', companyid: CompanyProductGridpage, first: 1000 }));
    }
  }, [status, dispatch]);
  ...
  const fetchMoreProducts = () => {
    dispatch(fetchProductpage({ after: endcursor, companyid: CompanyProductGridpage, first: 1000 }));
  };
